...
class MatchmakingPool(object):
    ...
    def _update(self):
        self.logger.info("Pool is now of size %d: %s" % (self.num_looking_for_game, self.pool))
    ...
    def add_player(self, player):
        ...
        if len(self.pool) >= self.MIN_NUM_PLAYERS:
            new_players = []

            while len(new_players) < 6 and len(self.pool) != 0:
                new_players.append(self.pool.pop())
